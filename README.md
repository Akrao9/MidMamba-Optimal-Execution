...
SB3 checkpoints are **`.zip`** (model) plus optional **`{stem}_vecnormalize.pkl`** (normalization stats) and **`{stem}.run_config.json`** (hyperparameters for eval). If the vecnorm file sits next to the `.zip` with that naming, `evaluate_execution.py` picks it up even without a run config. Loading the SB3 `.zip` and VecNormalize `.pkl` uses pickle-style deserialization, so pass `--trust-checkpoint` only for artifacts you created or otherwise trust. The JSON config is not cryptographically bound to the checkpoint; keep the model, VecNormalize stats, and config together as one artifact set. Evaluation reports include file hashes to make that set auditable.
