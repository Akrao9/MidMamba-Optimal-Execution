...
Across all full runs, the policy consistently performed best on the first held-out March sessions immediately after the validation period. The edge became mixed later in the month, which suggests the learned execution behavior transferred to nearby market conditions but degraded under later March regimes. This supports the main conclusion that a static policy is regime-sensitive and should be evaluated with rolling walk-forward retraining or regime-aware deployment.
